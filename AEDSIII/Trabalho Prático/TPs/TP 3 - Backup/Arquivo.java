...
public class Arquivo<T extends Registro> {
  ...
  public void FazerBackup(String VersaoBackUp) throws Exception {
    ...
    while (arq.getFilePointer() < arq.length()) {
      ...
      if (lapide == ' ') {
        ...
        byte[] dados = ReadBackup(l.getID());
        if (dados != null) {
          byte[] dadosCodificados = LZW.codifica(dados);

          Backup.salvarBackup(dadosCodificados, VersaoBackUp);
        }
      }
    }
  ...
  public static byte[] RecuperarBackUp(String nomArq) {
    // Define o diretório fixo
    String directory = "backup/";

    // Concatena o diretório com o nome do arquivo
    Path path = Paths.get(directory + nomArq);
    byte[] fileBytes = null;
  ...
  public byte[] ReadBackup(int id) throws Exception {
    T l = construtor.newInstance();
    byte[] ba;
    short tam;
    byte lapide;
